...
namespace camodocal
{
    //子类不会继承父类的构造函数！但是实例化子类时必须初始化从父类继承来的成员，因此会系统自动调用父类默认构造函数（指无参构造函数）或必须显式调用父类的有参构造函数
    //下面的“Camera::Parameters(PINHOLE)”语句就是在子类的构造函数中显式地调用父类的有参构造函数来初始化从父类继承来的参数
...
//将参数加入到 输出流ostream对象中，并返回其引用。使用方法就是cout<<PinholeCamera::Parameters类型对象，其中cout就是作为重载函数的第一个实参。
//另外，这里在定义重载函数时，为什么不用加类名限定了？？因为它是友元函数，它其实是属于类外部的普通函数！！！
std::ostream&
...
//如果子类是这种构造函数，那mParameters这个参数该如何初始化呢？mParameters在创建时可以被自动初始化（PinholeCamera::Parameters有定义无参的构造函数）
// PinholeCamera::Parameters的构造函数中有初始化m_modelType这个变量！
...
//这个函数是PinholeCamera类的构造函数，因此只有在创建PinholeCamera类对象时，才会同时创建mParameters并需要初始化它，
//这里是用有参构造函数初始化它（因此不存在重复初始化）！！这种专门的语法， 叫做构造函数初始化列表
PinholeCamera::PinholeCamera(const std::string& cameraName,
                             int imageWidth, int imageHeight,
                             double k1, double k2, double p1, double p2,
                             double fx, double fy, double cx, double cy)
    //下面这句括号内的就是PinholeCamera::Parameters类的构造函数（也就是初始化）所需的参数
...
PinholeCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P) const
{
    ...
    // Obtain a projective ray （带畸变的归一化平面坐标？）
    P << mx_u, my_u, 1.0;
}
...
PinholeCamera::readParameters(const std::vector<double>& parameterVec)
{
    ...
    //在PinholeCamera类的成员函数定义内，默认的Parameters类型就是其内部类的那个，若要用Camera类下的，则要加类名限定
    Parameters params = getParameters();
