...
CameraCalibration::CameraCalibration(const Camera::ModelType modelType,
 ...
 , m_verbose(false)
{
    //CameraFactory::instance()此句创建了一个动态的CameraFactory对象，并将智能指针m_instance指向它，返回m_instance
                            ...
                            std::vector<cv::Mat>& rvecs, std::vector<cv::Mat>& tvecs) const
{
    ...
    // Transform是Eigen库中的一个类，表示投影或者仿射变换矩阵
    ...
    for (size_t i = 0; i < m_imagePoints.size(); ++i)
    {
        for (size_t j = 0; j < m_imagePoints.at(i).size(); ++j)
        {
            ...
            //LossFunction指的是被优化函数的形式（自变量为costfunction)，如二次型，或者Cauchy型
            //注意，相机内参和相机外参（位姿）都是被优化的
        ...
        // LocalParameterization是EIgen中的纯虚类，EigenQuaternionParameterization是其子类，这里是用基类指针指向子类对象，即多态
        ceres::LocalParameterization* quaternionParameterization =
            new EigenQuaternionParameterization;
        //位姿是参与优化的，quaternionParameterization的作用是对旋转部分使用四元数的更新规则（具体待查明）
