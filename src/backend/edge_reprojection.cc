...
//    std::cout << pts_i_.transpose() <<" "<<pts_j_.transpose()  <<std::endl;
    ...
    //pts_i_是观测到该路标点的初始帧上的坐标，inv_dep_i为对应深度
    ...
    //为什么这里的视觉残差没有像崔华坤pdf中所讲的那样乘上正切投影的矩阵？？因为这里选择的是VINS-Mono中的用归一化坐标计算残差，而不是用球面坐标，导致优化形式不一样！！
    //另外，在VINS-Mono的代码中，这里残差有两种，一种就是这里的归一化坐标相减；另一种就是正则化坐标（模为1）相减
    //见 https://blog.csdn.net/weixin_37835423/article/details/111084263 
...
void EdgeReprojection::ComputeJacobians() {
    ...
    //reduce是因为残差中被减数需要除以z坐标而引入的？VINS-Mono中根据残差的定义不同，所得到的reduce也不同
    Mat23 reduce(2, 3);
