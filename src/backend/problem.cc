...
//IOFormat类存储一组控制矩阵打印方式的参数。这前四个参数分别设定 浮点数精度，禁用列对齐的标志（加快代码速度），同一行两个字符之间的分隔符，两行之间的分隔符
const static Eigen::IOFormat CSVFormat(Eigen::StreamPrecision, Eigen::DontAlignCols, ", ", "\n");

void writeToCSVfile(std::string name, Eigen::MatrixXd matrix) {
    //c_str()函数返回一个指向正规C字符数组的指针常量，内容与本string串相同（为什么要转化成C的格式？）
...
namespace myslam {
namespace backend {
    //使用日志时，只需要指明日志的级别和要输出的信息即可。格式：LOG(级别)<<"输出的信息";
...
void Problem::AddOrderingSLAM(std::shared_ptr<myslam::backend::Vertex> v) {
    if (IsPoseVertex(v)) {
        //OrderingId指的是某点在点集中是从第几维开始的，用于在雅可比矩阵中的定位
...
void Problem::ResizePoseHessiansWhenAddingPose(shared_ptr<Vertex> v) {
    //区别矩阵的Reshape和Resize，两者是不一样的。Reshape是指不改变矩阵元素个数的情况下，改变矩阵中元素的大小，简单例子如转置，2x3->3x2。
    //而Resize则是改变矩阵元素个数情况下，改变矩阵的大小规模，如2x3->4x4。
    //为什么只有pose有先验？
...
bool Problem::AddEdge(shared_ptr<Edge> edge) {
    ...
    //vertexToEdge_是用来由vertex id查询edge，由于一个vertex可能关联多个edge，因此vertexToEdge_类型为multimap
...
vector<shared_ptr<Edge>> Problem::GetConnectedEdges(std::shared_ptr<Vertex> vertex) {
    ...
    //equal_range 该函数返回值是两个Map类型的迭代器，分别指向满足条件（键值相等）的元素起始位置 和 该类元素结束位置的下一个元素的位置。
    auto range = vertexToEdge_.equal_range(vertex->Id());
    //vertexToEdge_类型是unordered_multimap，尽管unordered_multimap能保证不同的关键字无序，但是它还是把具有相同关键字的元素放在了一起。因此具有最早insert的key值的一组相同元素最靠前
...
bool Problem::Solve(int iterations) {
    ...
    while (!stop && (iter < iterations)) {
        ...
        int false_cnt = 0;
        while (!oneStepSuccess && false_cnt < 10)  // 不断尝试 Lambda, 直到成功迭代一步（如果连续失败10次，则退出计算）
        {
            // setLambda
...
//            }
            ...
            // 更新状态量 需要先更新，因为需要用来判断是否为好的一步计算（先验中的H_prior不需要更新，仅更新b_prior）
            UpdateStates();
...
void Problem::SetOrdering() {
    ...
    for (auto vertex: verticies_) {
        ...
        {
            //这个函数就计算了pose类和landmark类各自点集的维度
            AddOrderingSLAM(vertex.second);
        }

    }
    //下面就是计算pose类点和landmark点在总的slam点集中的维度位置
    if (problemType_ == ProblemType::SLAM_PROBLEM) {
        // 这里要把 landmark 的 ordering 加上 pose 的数量，就保持了 landmark 在后,而 pose 在前
        ulong all_pose_dimension = ordering_poses_;
        //这里只是对idx_landmark_vertices_中的点（second项）的ordering_id_进行修改，而idx_landmark_vertices_中每个元素的first项指的是点的id_（即第几个点，与维度无关）
...
//#ifdef USE_OPENMP
//     //下面的指令是OpenMP中的一个指令，表示接下来的for循环将被多线程执行，另外每次循环之间不能有关系。
...
//#endif
    for (auto &edge: edges_) {
        ...
        // TODO:: robust cost
        //下面两个量都是vector，其中verticies中是vertex类指针
        ...
        for (size_t i = 0; i < verticies.size(); ++i) {
            ...
            for (size_t j = i; j < verticies.size(); ++j) {
                ...
                // 所有的信息矩阵叠加起来
                //Eigen中某些矩阵操作可能会引起混淆问题（当左右有相同变量时），Eigen会默认使用去混淆操作
                //这里使用noalias()来声明不会出现混淆，用于计算提速
            ...
            //这里核函数的实现参照VIO课程任务3-1文件中的公式17，多了个预先给定的信息矩阵W（具体推导可以找资料）
    ...
    Hessian_ = H;
    b_ = b;
    //在solve期间，有多少次迭代，就需要多少次H矩阵构造，时间需要累加
    ...
    if(H_prior_.rows() > 0)
    {
        ...
        /// 固定位姿，这个应该只发生在初始帧？
        /// landmark 没有先验（为啥？marg掉之后，观测点应该也有先验吧？如果先marg被该帧观测的特征点，再marg位姿，则剩下的位姿点不会有多余信息，信息全在位姿矩阵上
        /// 这是VINS-Mono的策略，即只保留Pose的部分的先验
        for (auto vertex: verticies_) {
...
void Problem::SolveLinearSystem() {
    ...
    if (problemType_ == ProblemType::GENERIC_PROBLEM) {
        ...
        //采用LDLT分解（要求矩阵时正定或负定）来求解线性系统，QR分解则对矩阵没有要求，LU分解要求可逆
...
void Problem::UpdateStates() {
    ...
    // update prior
    //err_prior_为先验残差，通过先验b和先验雅可比可以解算
    if (err_prior_.rows() > 0) {
        ...
        //这里的ordering_poses_包含了加进来的最新帧，即优化时是窗口帧数+1，优化之后进行marg。
        //这里是在迭代优化期间，b_prior_ 是新的一次迭代时先验部分的b（即上一次的b 加上 -H*delat_x_，第二项是因为上一次优化后的b总是接近于0但又不完全是0）
        //这里具体的逻辑见 https://www.cnblogs.com/ilekoaiq/p/8836970.html
...
/// LM
//这个函数是选择LM算法中阻尼因子的初始值，见VIO课程文件3-1中的第16页
...
//此函数是LM算法中阻尼算子的更新策略，参照VIO课程文件3-1中17-20页
bool Problem::IsGoodStepInLM() {
    double scale = 0;
//    scale = 0.5 * delta_x_.transpose() * (currentLambda_ * delta_x_ + b_);
//    scale += 1e-3;    // make sure it's non-zero :)
    //scale的计算参照VIO课程文件3-1中的公式（11）
    scale = 0.5* delta_x_.transpose() * (currentLambda_ * delta_x_ + b_);
    ...
    if (err_prior_.size() > 0)
        //同样需要加上先验部分的误差，这里先验误差也是经过更新的了，见UpdateStates()函数
        tempChi += err_prior_.norm();
    tempChi *= 0.5;          // 1/2 * err^2

    double rho = (currentChi_ - tempChi) / scale;
    //isfinite()用于检查给定值是否为有限值，为什么要检查？会有什么问题？
 ...
 *
 */
//具体算法参照 https://blog.csdn.net/matrix_space/article/details/80467190
 ...
 *
 */
//pose_dim指的是所有pose点的总维度（第一个是外参位姿点，6维）
bool Problem::Marginalize(const std::vector<std::shared_ptr<Vertex> > margVertexs, int pose_dim) {

    SetOrdering();
    /// 找到需要 marg 的 edge, margVertexs[0] is frame pose, its edge contained pre-intergration and reprojection
    //每次只会marg一帧，因此这里的margVertexs中只有两个元素（pose和V_Bias状态向量）
    ...
    // 构建 简易的Hessian（半包含全部帧PRV和首帧相关的特征点），pose 的顺序不变，landmark的顺序要重新设定（排在pose的后面，所有特征点的开头）
...
//    std::cout << "pose dim: " << pose_dim <<std::endl;
    ...
    int ii = 0;
    //遍历和被marg帧相关的边，一条边就是一个残差，需要求得和这个残差相关的Vertex的雅可比，并得到H矩阵相关项
    ...
    if (marg_landmark_size > 0) {
        ...
        // Hmm 是对角线矩阵，它的求逆可以直接为对角线块分别求逆，如果是逆深度，对角线块为1维的，则直接为对角线的倒数，这里可以加速（每个小块之间的操作结果没有关系）
        MatXX Hmm_inv(MatXX::Zero(marg_size, marg_size));
        // TODO:: use openMP
        ...
        bpp = bpp - tempH * bmm;
        //下面这个赋值使得H_marg和b_marg降维了
        H_marg = Hpp;
        b_marg = bpp;
    }
    //这个Prior只是帧位姿的，特征点没有Prior。
    VecX b_prior_before = b_prior_;
    if(H_prior_.rows() > 0)
    {
        //这里加H_prior_和b_prior_的操作放在marg过程前面也可以，因为marg过程中对位姿部分的H只有加和操作，没有影响
        H_marg += H_prior_;
        b_marg += b_prior_;
    }
    ...
    //marg pose
    // index 大的先移动，即先移动VertexSpeedBias，再移动VertexPose。被marg的部分移到最后
    //如果有需要其实可以一次性marg多帧，但是上面只marg和首帧（或次新帧）相关的特征点，因此这里只marg一帧，有两个Vertex
...
//        std::cout << k << " "<<idx << std::endl;
    ...
    double eps = 1e-8;
    int m2 = marg_dim; //15维，一帧的VertexSpeedBias和VertexPose
    int n2 = reserve_size - marg_dim;   // 剩下的位姿变量的总维度
    Eigen::MatrixXd Amm = 0.5 * (H_marg.block(n2, n2, m2, m2) + H_marg.block(n2, n2, m2, m2).transpose());

    //SelfAdjointEigenSolver类（模板类）是对自伴矩阵进行特征值分解，saes是该类的对象，Amm是初始化参数
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> saes(Amm);
    //Eigen中的基本函数 https://blog.csdn.net/u012541187/article/details/53420432
    //Eigen中的array()函数是对矩阵元素的逐个操作，结果维度和原矩阵相同（涉及到多个矩阵时，要求维度都相同）
    //(R.array()<s).select(P,Q)表示的是(R.array() < s ? P : Q),也是逐元素的操作，P和Q都是标量
    //先奇异值分解，再求逆，这时求你只需要对中间的对角阵求逆，即对角线逐元素取逆
    ...
    //下面就是根据计算出来的H_prior_反推出先验雅可比和先验残差
