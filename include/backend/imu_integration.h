...
public:
     ...
     * @param bg
     */
    //explicit关键字指定构造函数或转换函数为显式, 即构造函数的参数不具有隐式类型转换和复制初始化。
    //构造一个预积分时一定得需要给定Bias的初始值，等到优化估计之后再更新
...
private:
    ...
    //const修饰的变量可以在运行时才初始化，而constexpr则一定会在编译期初始化。 constexpr才是名符其实的常量，所有的constexpr都是const。
