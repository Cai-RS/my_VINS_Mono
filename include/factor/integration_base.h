//此文件为imu预积分，建立前后帧的imu误差的传播，计算预积分的残差。IntegrationBase()初始化时设置初始时刻的估计不确定性方差为0（计算残差时需要更新），初始时刻的雅可比为单位矩阵
//类、inline函数和编译时值已知的const对象可以在头文件中定义，这是因为遵守“单一定义规则”(One-Definition Rule, ODR)。只包含类定义的h文件可以被多个源文件加载，不会造成重复定义
//成员函数在类内定义时，实际上是被当作了inline函数，这样可以加快运行速度，但是增加了空间开销
#pragma once
...
//这里是两帧相机之间的imu数据点的预积分，因此每两帧之间就会有一个此类的对象（每次初始化只需要a、w和两个bias的初值）
class IntegrationBase
{
  public:
    IntegrationBase() = delete;
    //设置两个imu时刻的 imu数据是相同的，因为t=0时刻的imu数据不可知
    ...
    //重新进行预积分，这里并不是因为迭代优化后进行重新传播（这非常耗时），而纯粹是因为意外改变了bias的初始值（或者说bias的变化太大了，无法用线性化公式进行更新）
    ...
    {
        //这里的result_指的是新的时刻即k+1时刻，delta_p指的是i时刻的body相对于k时刻body坐标系的旋转（从k时刻开始，因此初始化为单位实四元数）
        ...
        if(update_jacobian)
        {
            ...
            //step_jacobian = F;
            //step_V = V;
            //jacobian的初始值为单位矩阵，那么这个矩阵是对什么进行求导得到的呢？
            jacobian = F * jacobian;
    ...
    //给定估计出的理论值，求取残差（这里i、j应该不是指前后两个时刻的，而是指整个预积分（即两个相机帧之间）的首尾时刻，因jacobian是已经全程传播过的了）
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        //jocobian里面PVQ增量误差对i时刻的bias误差的雅可比，直接被拿来作为PVQ增量对i时刻的bias的雅可比（如崔华坤推导中式子20下面的话所述）
        Eigen::Vector3d dba = Bai - linearized_ba;
        Eigen::Vector3d dbg = Bgi - linearized_bg;
        // 给定Bias的增量，通过线性化的方式更正预积分（因为预积分值跟bias有关，所以待估计值更新时，预积分值也应该更新）
