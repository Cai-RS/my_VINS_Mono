...
public:
    ...
    public:
        ...
        //此为运算符=的重载
        // C++允许在类的定义中使用本类的指针或引用，但不能在定义内部创建本类对象
        Parameters& operator=(const Parameters& other);
        friend std::ostream& operator<< (std::ostream& out, const Parameters& params);

        //它的成员变量其实还包括了从Camera::Parameters继承来的五个参数，所以在它的构造函数中需要将他们也作为参数（目的是为了初始化）
    private:
        double m_k1;
        double m_k2;
        double m_p1;
    ...
    */
    ...
    //对抽象类中同名虚函数的重新实现，这里是声明
    // ModelType该枚举类型是定义在Camera类中的，因此这里需要加域名
...
private:
    //创建Parameters类对象mParameters，由于Parameters类有无参构造函数，因此若无赋值，mParameters也是可以初始化的
    //在类的定义中创建的类对象会被初始化吗？答案应该是否定的！应该是创建PinholeCamera类对象时才会同步创建mParameters这个成员，并且此时才需要初始化它
                            ...
                            Eigen::Matrix<T, 2, 1>& p)
{
    ...
    // Transform to model plane 归一化坐标
    T u = P_c[0] / P_c[2];
    T v = P_c[1] / P_c[2];
